...
The official codes for our paper "[Linguistic-Aware Patch Slimming Framework for Fine-grained Cross-Modal Alignment](https://github.com/CrossmodalGroup/LAPS)", which is accepted by the  IEEE/CVF Conference on Computer Vision and Pattern Recognition, 2024.
...
We have prepared the texts of two datasets in  `data/` floder, you need to download the images of the datasets. 
The f30k images can be download in [flickr30k-images](https://www.kaggle.com/datasets/hsankesara/flickr-image-dataset). The coco images can be download in [train2014](http://images.cocodataset.org/zips/train2014.zip), and [val2014](http://images.cocodataset.org/zips/val2014.zip).
The final data is organized as follows:
...
## Training
We first set up the **arguments** for the model training. 
Detailed information about the arguments can be found in ```arguments.py``` .


- `--dataset`: the chosen datasets, e.g., `f30k` and `coco`.
- `--data_path`: the root path of datasets, e.g., `data/`.
- `--multi_gpu`: whether use the multiple GPUs (DDP) to train the models.
...
The models need **about 20,000 GPU-Memory** (one 3090 GPU) when batch size = 64 and **about 40,000 GPU-Memory** (one A40 GPU) when batch size = 128.
You need to modify the batch size according to the hardware conditions.
And we provide multi gpu training programs. 

Besides, considering the GPU-memory pressure, we not integrate the gumbel-softmax sampling (score prediction network) in the patch selection process. The performances are not affected much but the GPU-memory is reducing a lot.
...
## Evaluation
Run ```eval.py``` to evaluate the trained models on f30k or coco datasets. You need to specify the model paths in ```eval.py```.
...
We provide the training logs and model checkpoints, and the detail parameters setting can be found here.
...
|MSCOCO-5K |ViT | 56.1 | 83.9 | 43.9 | 73.6 |[Link](https://drive.google.com/drive/folders/1C0XZ7FAoq47huy6gItcIV4XqudRVfe3n?usp=sharing)|
